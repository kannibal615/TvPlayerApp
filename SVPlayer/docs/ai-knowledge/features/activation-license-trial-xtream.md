...
- `PlaylistWeb`: profil Android reserve aux playlists envoyees depuis `/playlist/` ou le QR web. `XtreamAccountManager` met a jour ce profil si son nom est toujours `PlaylistWeb`, y compris lorsque le formulaire web demande explicitement de creer un profil portant ce nom; un envoi unique ne peut donc plus produire `PlaylistWeb (2)`. Si l'utilisateur renomme le profil reserve, le prochain push web cree un nouveau profil `PlaylistWeb`.
