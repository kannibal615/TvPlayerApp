...
- Les dossiers virtuels Favoris et Historique ne sont rendus que s'ils contiennent au moins un element. S'ils deviennent vides pendant qu'ils sont selectionnes, l'ecran revient sur `ALL`.
- Le changement de dossier, de filtre ou de recherche Live TV arrete explicitement la session de preview avant de charger la nouvelle selection, afin qu'aucun flux masque ne continue en arriere-plan.
- Depuis le 2026-07-21, les libelles des sous-dossiers des plateformes sont geometriquement alignes a gauche, y compris en arabe; les logos parents restent centres et l'indentation/D-pad sont inchanges.
- La fiche Detail Serie charge le poster portrait de la saison selectionnee via `/tv/{series_id}/season/{season_number}`, le garde en cache memoire par serie/saison/langue et retombe sur le poster general. Le poster occupe l'extremite droite, la description Saison est compacte en bas et la liste affiche cinq episodes complets tout en restant scrollable/focusable.
...
Depuis le 2026-07-21, les fiches Detail Film et Detail Serie reutilisent `ui/home/TvHeader.kt` via `DetailHeader`, sans header visuel parallele. La fiche Detail Serie remplace les actions hero `Reprendre`/`Favoris` par les boutons de saisons disponibles, aligne le poster de la saison en haut au niveau du titre, conserve la liste des episodes en bas a gauche et agrandit la description de saison dans l'espace droit libere. La gestion favoris reste disponible dans le panneau Preview Series du catalogue.
...
Home charge ses donnees legeres hors splash: slides, historique recent limite a `10` et tendances films/series `10 + 10` depuis Room. Chaque calcul est identifie par `profileId + catalogRevision` et passe par `flatMapLatest/mapLatest`; un resultat tardif d'un ancien profil ou d'une ancienne revision ne peut plus etre publie. Au changement de profil, historique, tendances, compteurs et previews sont vides immediatement, puis la revision Room courante est rechargee.
...
Depuis le 2026-07-17, la selection Series Home normalise les notes sur `5`, exclut toute note inferieure a `3/5`, priorise un pool recent borne puis les meilleures Series globales et repare `trending_media` si la selection persistee est obsolete. Films conserve la selection persistante existante; aucun schema Room ni endpoint n'est ajoute.

Depuis le 2026-07-17, l'enrichissement de l'historique episode reconnait aussi `Serie/Series <id>` comme titre genere. Il resout la serie par episode, `parentContentId`, puis identifiant extrait. Si les episodes ne sont pas encore en Room, `CatalogRepository.getOrFetchSeriesEpisodes(seriesId)` lit le cache local puis utilise `get_series_info`, persiste les episodes sous le profil capture et refuse de rendre le resultat si le profil actif a change. Continue Watching et le lecteur reutilisent ce resolver pour restaurer mini-player, vrai titre de serie, saison/episode et voisins apres redemarrage, sans migration Room.

Le lecteur plein ecran capture aussi le `profileId` qui a ouvert la session. Ses lectures et sauvegardes de progression utilisent cet identifiant jusqu'a la liberation du player: une sauvegarde de sortie terminee apres un changement de profil ne peut donc plus ecrire l'ancien contenu sous le nouveau profil actif.
...
Depuis le 2026-07-09, `XtreamAccountManager` porte des profils playlist locaux et `PlaylistProfile` reste la source de verite des identifiants. Le pipeline d'activation partage invalide les caches memoire, publie `catalogRevision` et ne lance qu'une synchronisation eventuellement due pour le changement courant; Room reste isole par `profileId`. Home observe la revision et `SyncStatus`: une reussite recharge compteurs/tendances/historique avant l'etat pret, tandis qu'une erreur termine explicitement le chargement sans skeleton infini.
...
Depuis le 2026-07-10, la creation d'un nouveau `PlaylistWeb` ne remplace plus le profil actif existant. Dans Who's Watching, OK cree une requete `requestId + profileId`, active localement le profil puis attend la synchronisation eventuellement due et le rechargement Home de la revision courante avant de terminer l'animation. Un profil possedant deja un `sync_state` reussi et du contenu Room reste cache-first et ne resynchronise pas avant l'echeance configuree.
...
Depuis le 2026-07-17, les cards episodes de Continue Watching portent le badge superieur `SERIE` en plus du badge `Sx Ex`. L'overlay episode ajoute apres `Suivant` une action focusable `Series details` / `Detail serie`; elle sauvegarde la progression, libere Media3, retire la route player puis ouvre `series_detail/{seriesId}`.
...
- 2026-07-18: `PlaylistProfile.selectedContentPrefixes` filtre l'import Live/Films/Series sur les codes de 2-3 lettres places au debut du titre (`FR -`, `AR:`, `EN |`, `[UK]`) et sur le code special entre crochets `[X]`/`[x]`, normalise en `X`. Une selection vide garde tout; une selection multiple applique un OU, exclut les titres sans prefixe et conserve uniquement les categories d'origine non vides. Le filtre Kids s'applique ensuite. Les codes detectes ou saisis manuellement enrichissent la liste locale, et `CatalogSyncFingerprint` invalide un catalogue quand la source resolue, le type ou les filtres changent, y compris pour un Kids partageant les identifiants Admin.
- 2026-07-18: les recherches Films/Series conservent la saisie brute avec ses espaces dans l'etat UI et ne font `trim()` qu'au moment du filtrage/repository. Le header Channels utilise un `COUNT(*)` Room avec le meme couple categorie/recherche que la page chargee, independamment de la pagination.
- 2026-07-18: le mini-player et le fullscreen Live reutilisent une unique `LivePlaybackSession`/instance ExoPlayer. Le premier niveau Live suit la maquette: gradient bas, logo, titre/EPG, precedent, pause, suivant, luminosite et sortie. Depuis le 2026-07-22, seul Menu ouvre les options secondaires; D-pad Bas reste reserve au zapping.
- 2026-07-22: apres un zapping Haut/Bas, le player Live affiche pendant 3 secondes un guide gauche transparent et non interactif, borne a deux chaines precedentes, la chaine courante et deux suivantes. La fenetre vient de Room, reste dans la categorie courante, suit l'ordre numero/nom/streamId et ne boucle pas aux extremites. Le rendu compact ne montre que logo et nom, sans numero ni cadre de liste/logo; une barre laterale et un fond leger identifient la lecture courante. L'etat du guide est conserve entre deux routes de zapping et chaque nouveau zap prolonge son delai sans disparition intermediaire.
- 2026-07-21: la persistance Room des dossiers Live TV, Films et Series ne remplace plus une liste valide par un snapshot Xtream invalide lorsque des contenus sont encore conserves. Un cache ayant des medias sans dossiers est considere incomplet et force la prochaine synchronisation automatique, y compris avec la frequence manuelle.
- 2026-07-22: les sections Xtream standard Live TV, Films et Series valident leurs dossiers fournisseurs avant toute ecriture puis remplacent dossiers et medias dans une transaction Room unique. Une relecture immediate est tentee si l'endpoint categories est vide; si le snapshot reste inexploitable, la synchronisation echoue en conservant integralement le dernier catalogue coherent.
- Diagnostic categories: le tag Logcat `SVCatalogCategories` mesure sans identifiants ni libelles les lectures cache/Room, les snapshots ecrits ou rejetes et la projection finale des ViewModels Live TV, Films et Series. Les compteurs `raw`, `allowed`, `state` et `rendered` isolent une perte entre Room, filtrage local et rendu UI.
- Le meme tag trace aussi les dossiers plateformes VOD (`platform_grouping`) et le chargement de `ALL` (`all_category_order`) : nombre de categories reelles, sous-dossiers regroupes, ordre exploite, pagination et repli eventuel. Les dossiers synthetiques Netflix/Prime/Apple/Disney restent exclus des requetes Room ; leurs enfants sont conserves dans l'ordre des categories affichees.
- Correctif 2026-07-22 : dans les trois fonctions `withSpecialCategories`, le filtrage fournisseur est calcule avant `buildList`. Sans cette variable, `filterNot` visait le recepteur du builder deja rempli des seuls dossiers speciaux, ne laissant que `ALL` (puis eventuellement Favoris/Historique) malgre des dossiers Room valides.
