...
Depuis le 2026-07-03, le startup Compose de `MainActivity` ne lance plus la synchronisation complete et ne prechauffe plus les categories/pages catalogue. Il fait seulement les controles activation/source/connexion, lit les compteurs locaux et la fraicheur `SyncState`, puis publie une demande `StartupCatalogWorkRequest` vers Home: `Synchronize` si la frequence utilisateur l'exige, `LoadLocal` si un catalogue Room existe deja, sinon aucun travail.
...
- Le chargement local post-startup est orchestre par Home, section par section, avec categories/counts et premieres pages locales bornees (`96` Live, `72` Films, `72` Series). Les details tendances Home sont charges depuis les tendances sauvegardees par section; ce chargement ne doit pas etre confondu avec une synchronisation reseau.
...
- Le premier chargement local ne doit plus etre dans `MainActivity`: categories/counts, premieres pages locales bornees `96/72/72` et tendances sauvegardees sont maintenant geres par Home avec overlays de progression. Ne pas remettre les details tendances Xtream, EPG reseau ni snapshots complets Movies/Series dans `MainActivity` pour les tres gros catalogues.
...
- 2026-07-03: deplacement de la synchro automatique et du chargement catalogue local hors `MainActivity`; Home gere maintenant les cards Live TV / Films / Series avec overlay sombre progressif, blocage telecommande, phases `WAITING/RUNNING/IMPORTING/LOADING_TRENDS/COMPLETED/ERROR` et chargement des tendances sauvegardees par section.
