...
Depuis le 2026-07-07, `Media` ajoute une categorie `Media prives` separee des fichiers locaux. Android consomme uniquement `PrivateMediaRepository` vers les endpoints SmartVision `api/media/private/*`; aucun endpoint Eporner n'est appele depuis l'APK. Le backend normalise les reponses provider en DTO internes et renvoie `EMBED`/`PAGE_ONLY`/`UNAVAILABLE` tant qu'aucun flux HLS/MP4 officiel n'est disponible. La fiche detail privee affiche les metadonnees et garde `Lecture indisponible` si le backend ne fournit pas un stream natif compatible.
