...
Depuis le 2026-07-21, Movies et Series reutilisent les memes filtres de categories/drapeaux que Live TV. Sous un filtre, leur dossier `ALL`, leur recherche et leur pagination Room utilisent uniquement les identifiants de categories correspondants; Favoris et Historique sont masques comme dans Live TV. Les categories fournisseur restent intactes en base.

Dans Movies et Series, les categories contenant un mot delimite `NETFLIX`, `PRIME`, `APPLE` ou `DISNEY` sont projetees dans des dossiers virtuels non vides, epingles apres Historique dans cet ordre. Un seul dossier est ouvert a la fois; ses enfants conservent identifiant, libelle, compteur, ordre et chargement existants. Les parents n'ont pas de compteur, ne chargent aucun contenu et utilisent les drawables fictifs `brand_*_placeholder`, remplacables sans modifier la logique.
