...
- `Reset Xtream` supprime ensemble la configuration PlaylistWeb et l inventaire `device_playlist_profiles`, puis force `devices.xtream_status = missing`. Il ne doit pas recalculer le statut depuis l inventaire qu il est justement charge de remettre a zero.
- Les uploads de fond admin restent stockes sous forme de chemin public `/assets/uploads/personalization/...`; Android les normalise avec `ACTIVATION_BASE_URL` avant chargement Coil, tandis que les URL http(s) completes restent inchangees. `AppConfigViewModel` est rafraichi au retour au premier plan afin qu un fond change depuis l admin soit repris sans redemarrage de l application.
