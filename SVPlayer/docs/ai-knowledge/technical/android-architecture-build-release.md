...
- Le chargement est rendu par un `AnimatedVectorDrawable` bleu/cyan indetermine dans une vue Android dediee: son groupe vectoriel tourne lineairement en boucle de `800 ms` jusqu'a la sortie du Splash. La vue demarre/arrete elle-meme son animation a l'attachement et au detachement; les changements de statut Compose ne pilotent, ne reconstruisent ni ne relancent cette rotation decorative. Le statut reste sous le cercle; il n'existe plus de barre lineaire.
- Le contenu initial est precompose pendant le splash. Pour Who's Watching, le signal `initial surface ready` attend le layout d'une vraie card puis une frame supplementaire; le handoff ne depend donc plus du seul conteneur du picker. Les cards restent precomposees a une opacite imperceptible sous le splash, puis sont revelees ensemble par un fondu de `140 ms`, sans apparition echelonnee.
...
- Le theme systeme `Theme.SVPlayer.Splash` et `StartupExperience` reutilisent `startup_cinema_background.png` en plein ecran et `smartvision_logo_wide.png`: la preview Android et la premiere frame Compose gardent donc la meme composition sans second ecran ni `SplashActivity`.
- Le startup applicatif suit les phases visuelles `LogoOnly -> Loading -> TransitionOut`. `LogoOnly` dure au moins `250 ms`; la zone de chargement devient visible apres `150 ms`. Le splash reste au-dessus de `AppNavigation` pendant le precompose, puis se retire par un fondu de `140 ms`.
- `StartupProgressSnapshot` derive les quatre etapes locales reellement terminees pour le statut affiche. La quatrieme etape correspond a la premiere frame de la surface applicative initiale; aucun ETA ou compteur artificiel n'est affiche et les statuts passent par `SmartVisionStrings` en anglais/francais.
- `MainActivity` attend la premiere frame Compose avant de lancer les checks startup, puis compose `AppNavigation` sous `StartupExperience` et attend une card Who's Watching effectivement placee suivie d'une frame avant le handoff. Un timeout `2500 ms` evite tout blocage et une courte tenue de fin conserve la lisibilite du statut.
...
- Le splash systeme affiche le logo wide plus petit et plus haut; l'overlay Compose place le spinner decoratif centre sous ce logo et conserve le statut sur une ligne distincte.
