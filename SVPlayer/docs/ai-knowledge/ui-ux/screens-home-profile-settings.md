...
Les ecrans actifs sont routes depuis `ui/navigation/AppNavigation.kt`. Le header principal expose Home, Live TV, Movies, Series, Media et YouTube en onglets centraux icon-first. Ils utilisent six `VectorDrawable` 24 x 24 sans fond integre: cinq contours monochromes blancs et le logo YouTube rouge/blanc en couleurs originales. Hors focus, l'empreinte reste transparente, sans cadre ni label; au focus, la cible fixe `58 x 44 dp` passe a `1.15x` sans remesure et affiche en parallele halo bleu partage mobile, barre `28 x 3 dp`, opacite complete et label. L'espacement de `12 dp` absorbe le zoom sans deplacer les voisins. La route active n'affiche plus de selection persistante quand le focus est dans le contenu. Les actions globales restent dans le bloc droite, puis la date/heure non focusable sans separateur vertical dedie. L'horloge et ses secondes sont configurables globalement dans Personnalisation et reagissent immediatement. Le bouton licence/Premium n'est plus rendu dans le header depuis le 2026-07-20; le dialogue Premium reste appele par les features verrouillees. Les ecrans de contenu affichent un QR Xtream si aucun compte n'est configure. Si un compte Xtream existe mais que la verification rapide echoue, Home reste accessible et les entrees Live TV / Movies / Series sont bloquees par popup et overlay. Pendant une verification Xtream en cours, la navigation catalogue peut rester temporairement protegee si aucun etat connecte valide n'est deja connu, mais Home et le header ne doivent afficher les warnings/overlays "Connexion indisponible" qu'apres un echec Xtream confirme.
