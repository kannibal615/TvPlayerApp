...
- Home affiche ses donnees a partir de petits jeux locaux: slides caches, historique recent limite a `10`, et tendances films/series `10 + 10` aleatoires depuis Room, hors adulte, avec uniquement les metadonnees locales disponibles. Le splash ne bloque plus sur les details Xtream/backdrops, ne precharge plus Home et ne publie plus de chargement catalogue; Home relance un refresh tendances cache-first apres sa premiere frame.
...
- Sur Home, les tendances Films/Series utilisent les posters locaux disponibles. Les details Xtream et `backdropUrl` ne sont plus demandes pour construire les lignes tendances.
