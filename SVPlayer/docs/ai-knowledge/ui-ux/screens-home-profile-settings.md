...
Apres attribution du focus initial ou rafraichissement depuis le bouton Home du header, Home restabilise aussi le scroll a zero au frame suivant. Un spacer structurel non scrollable de `24 dp` separe le header du viewport Home; ce viewport est clippe a ses limites. Le hero ne peut donc plus etre dessine sous le header, meme si le scroll est restaure ou anime par le focus. Les positions Continue Watching/Tendances incluent le bloc hero et son espacement inferieur.
...
- Les cards categories, Continue Watching et Tendances partagent le halo bleu des cards Who's Watching. Les `LazyRow` Historique/Tendances gardent `20 dp` de padding horizontal interne pour que l'echelle et l'ombre de la premiere card ne soient pas coupees par le bord gauche du viewport, tout en conservant cinq cards paysage completes; la bordure et la couleur de focus configurables restent conservees.
