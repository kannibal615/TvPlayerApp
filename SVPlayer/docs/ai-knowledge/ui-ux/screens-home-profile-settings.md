...
Tous les ecrans principaux reutilisent le meme `ui/home/TvHeader.kt` et le meme shell exterieur: `25 dp` de marge horizontale, `14 dp` de marge verticale et `12 dp` entre header et contenu. Home garde son hero dans le flux sous ce shell et remet son scroll vertical a zero lors d'un retour cible vers le header.
Apres attribution du focus initial ou rafraichissement depuis le bouton Home du header, Home restabilise aussi le scroll a zero au frame suivant. Le viewport scrollable conserve un padding superieur non scrollable identique au spacer hero/categories afin que `bringIntoView` ne puisse ni supprimer l'espace sous le header ni tronquer le hero; les positions Continue Watching/Tendances incluent le bloc hero et son espacement inferieur.
...
- Les cards categories, Continue Watching et Tendances partagent le halo bleu de `25 dp` des cards Who's Watching, applique avant le clip pour rester visible autour du contour; la bordure et la couleur de focus configurables restent conservees.
...
- Le hero Home ne contient plus de bouton et n'entre jamais dans le parcours D-pad. Son index est reinitialise au changement de profil/du mode Kids et chaque image distante ou locale utilise l'asset embarque correspondant comme placeholder/error fallback.
