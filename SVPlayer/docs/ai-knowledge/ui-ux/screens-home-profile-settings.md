...
- Une reprise episode affiche `SERIE` en haut et `Sx Ex` en bas. Si les episodes ne sont pas locaux apres redemarrage, Home recharge `get_series_info` sous le profil capture avant de construire le mini-player et les metadonnees.
...
- `Info profil` ne contient aucun CRUD ni identifiant: profil actif, selection explicite, compteurs Live/Films/Series/elements masques, derniere synchronisation et action de synchronisation. Un changement n'est applique qu'apres OK, invalide les caches existants et projette le profil dans les flows legacy. Depuis Info profil comme depuis Who's Watching, il cree la meme requete `requestId + profileId`, affiche la transition du picker et ne la termine que lorsque l'activation et Home prete correspondent a cette requete.
...
- Apres le splash et avant Home, `ProfilePickerScreen` affiche tous les profils configures dans une `LazyRow` sans limite artificielle. OK active immediatement le profil et invalide les caches Home/Xtream, mais le reveal final attend les donnees Home et la synchronisation eventuellement due pour la requete courante. Le pipeline est partage avec `Info profil > Changer de profil`; Home remplace tout `HomeUiState.profileId` obsolete par un etat cible vide/loading avant rendu, donc aucun compteur, historique ou tendance de l'ancien profil ne peut apparaitre. La synchronisation ne se lance que si le catalogue manque ou si 24h/48h est echu; `A chaque demarrage` n'est jamais rejoue par un retour Home.
...
- Who's Watching identifie chaque clic par `requestId + profileId`. La transition conserve ses animations existantes mais ne se termine que si cette requete est encore courante, le profil actif correspond, l'activation/synchronisation est terminee et Home a charge la meme revision catalogue.
...
- 2026-07-17: Who's Watching anime le profil choisi vers le centre avec zoom, disparition des autres cartes et loader circulaire sur l'avatar jusqu'au chargement complet du Home reel precompose en arriere-plan. La sortie revele Home puis deplace l'avatar vers les coordonnees mesurees du bouton profil du header; cet avatar remplit desormais le bouton sans fond colore intermediaire.
