...
- L'espacement vertical Continue Watching -> Trending Movies et Trending Movies -> Trending Series utilise la constante unique `HomeContentSectionSpacing = 10.dp`; dimensions et gaps horizontaux des cards sont inchanges.
...
- Info compte commence par `Info compte`, puis `Controle parental`, `Synchronisation`, Historique/Aide/Parametres. `Synchronisation` est l'unique ecran de configuration du demarrage, de la synchro en arriere-plan et de la frequence; l'ancien doublon Settings a ete retire. Sa colonne droite conserve le cadre `ProfilePanel` et affiche deux cartes superieures `Options generales` / `Frequence`, puis un `Resume de synchronisation` pleine largeur avec frequence, derniere synchro, profil actif et serveur resolu. Le panneau parental conserve le header et le menu gauche de Profile, puis isole dans la zone droite les cartes Activation, Code PIN, Mots-cles et Resultats. Sa `LazyColumn` ne fait defiler que cette zone. `Licence SmartVision` n'y est plus affichee. Le contenu `Appareil et catalogue` reste integre et focusable pour le scroll TV.
...
- Apres le splash et avant Home, `ProfilePickerScreen` affiche tous les profils configures dans une `LazyRow` sans limite artificielle. Quand toutes les entrees tiennent a l'ecran, leur largeur totale calculee centre exactement le groupe. Les avatars profils/actions partagent un diametre unique, une meme zone verticale et le meme composant de libelle pour aligner profils reels, `Add kids profile` et `Add profile`; les deux actions utilisent leurs PNG dedies `avatar_action_add_kids` et `avatar_action_add_profile`. OK renforce brievement la carte choisie, active le profil, invalide seulement les caches memoire existants puis ouvre Home; aucune synchronisation Xtream/EPG n'est lancee depuis le picker. Home conserve la decision de synchronisation apres son premier rendu. Le focus initial cible le profil actif, sinon ADMIN, sinon le premier profil. Le crayon est centre sous chaque carte reelle, hors du cadre; Bas le cible et Haut revient a la carte. Le formulaire restaure la cible d'origine a sa fermeture.
...
- Le panneau parental actif est pilote par `ParentalControlViewModel`. Ses tailles suivent `ProfilePanel` / `DeviceCatalogInlineSection`: titre `TitleS`, titres de sections `Label`, icones compactes et toggle `54 x 26 dp`. Les mots-cles sont ordonnes, normalises et affiches en quatre colonnes sur grande largeur, au minimum trois colonnes sur la largeur TV normale observee, puis deux/une seulement sur espace etroit. `Filtering keywords` garde son titre sur une ligne distincte des controles. Le champ reste en lecture seule au simple focus; OK/clic ouvre l'edition et le clavier. Add ajoute immediatement une card, Edit ouvre la saisie pre-remplie et Delete exige un `TvConfirmationDialog`. Les resultats affichent uniquement les dossiers et contenus reels du cache Room du profil actif, avec compteurs exacts et pagination par lots de `40`.
...
- Notifications reprend le layout TV a deux colonnes: menu gauche `250 dp` (Toutes, Mise a jour, Playlist, Information importante, Historique) et liste flexible a droite. Les badges rouges comptent uniquement les non-lues; une notification rejoint l'historique apres ouverture et y reste jusqu'a purge admin.
...
- `ParentalControlViewModel` combine settings, profil actif et `catalogRevision`; `RoomParentalCatalogRepository` calcule compteurs, dossiers et elements sans charger le catalogue complet en memoire.
