...
- Le panneau parental actif est pilote par `ParentalControlViewModel`. Ses tailles suivent `ProfilePanel` / `DeviceCatalogInlineSection`: titre `TitleS`, titres de sections `Label`, icones compactes et toggle `54 x 26 dp`. Les mots-cles sont ordonnes et normalises. Les resultats sont un snapshot Room v17 persistant, regenere seulement apres changement de mots-cles/profil ou prochaine synchronisation catalogue. La colonne gauche selectionne un dossier masque; OK/clic charge exclusivement ses elements dans la colonne droite, avec compteurs exacts et pagination par lots de `40`.
...
- `ParentalControlViewModel` combine settings, profil actif et `catalogRevision`; `RoomParentalCatalogRepository` reutilise le snapshot persistant valide ou le regenere sans charger le catalogue complet en memoire.
