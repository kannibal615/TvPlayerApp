...
- Info compte: licence, device, mode d'utilisation, expiration, profils playlist, compte Xtream, lien M3U, URL EPG, source active exclusive, QR achat/config. L'ecran reutilise le header principal de l'application, donne le focus initial au menu `Licence SmartVision`, et garde la section droite plus large/compacte pour servir de modele aux futurs ecrans profil/parametres. Dans Info compte, le bouton Synchroniser ouvre un popup de confirmation avec compteurs Live TV / Films / Series; apres succes ou erreur, Retour ferme le popup, ouvre Appareil et catalogue et y remet le focus.
- Info compte > Profils: OK sur une ligne profil selectionne uniquement le panneau de details existant en bas, sans popup. Ce panneau reutilise les cartes `Compte SmartVision`, `Lien M3U`, `URL EPG` et `Synchroniser/Supprimer`, alimentees par le profil selectionne; le toggle ON/OFF a droite de la ligne active le profil et reste focusable avec la telecommande via D-pad droite/gauche.
- Apres splash et avant Home, un ecran `ProfilePickerScreen` affiche les profils configures sous forme d'avatars avec premiere lettre et couleur stable. OK sur un profil l'active immediatement puis ouvre Home; la synchro eventuelle ne bloque pas le clic. `Add Profile` et `Manage profiles` quittent le picker puis ouvrent Info compte > Profils apres montage du `NavHost`.
