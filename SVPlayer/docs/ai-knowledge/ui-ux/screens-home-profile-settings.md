...
- Info compte > Profils: OK sur une ligne profil selectionne uniquement le panneau de details existant en bas, sans popup. Ce panneau reutilise les cartes `Compte SmartVision`, `Lien M3U`, `URL EPG` et `Synchroniser/Supprimer`, alimentees par le profil selectionne; le toggle ON/OFF a droite de la ligne active le profil et reste focusable avec la telecommande.
- Apres splash et avant Home, un ecran `ProfilePickerScreen` affiche les profils configures sous forme d'avatars avec premiere lettre et couleur stable. OK sur un profil l'active puis ouvre Home; si son catalogue local existe deja, l'app ne resynchronise pas.
