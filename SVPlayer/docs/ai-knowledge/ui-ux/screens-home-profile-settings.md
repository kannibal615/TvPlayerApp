...
- Sur Home, les cards tendances films/series utilisent `TrendingContentRow`: portrait par defaut, focus court sans player, stabilisation `1s`, ancrage gauche de la card focussee, transformation 16:9 avec la meme transition visuelle que Continue watching, crossfade vers backdrop TMDB, fallback poster floute si aucun backdrop, puis mini-player YouTube en mode preview si TMDB fournit un trailer. Les tendances ne jouent plus le flux Xtream d'origine; sans trailer TMDB, la card reste poster/backdrop-only.
...
- Info compte: licence, profils playlist, compte Xtream, lien M3U, URL EPG, source active exclusive, QR achat/config, puis contenu `Appareil et catalogue` integre en bas de l'ecran. L'ecran reutilise le header principal de l'application, donne le focus initial au menu `Licence SmartVision`, et garde la section droite plus large/compacte pour servir de modele aux futurs ecrans profil/parametres. Le menu gauche `Appareil et catalogue` est supprime; son contenu integre est focusable mais non cliquable pour permettre le scroll TV. Le menu `Mode d'utilisation` est supprime.
- Info compte > Profils: OK sur une ligne profil ouvre/ferme le panneau de details existant en bas, sans popup. Ce panneau reutilise les cartes `Compte SmartVision`, `Lien M3U`, `URL EPG` et `Synchroniser/Supprimer`, alimentees par le profil selectionne; le toggle ON/OFF a droite de la ligne active le profil et reste focusable avec la telecommande via D-pad droite/gauche. Le header du detail rend l'avatar focusable pour ouvrir un popup de 10 avatars standards, puis `Enregistrer` ferme le popup; le stylo du nom ouvre le formulaire profil pre-rempli. Le changement de profil invalide les caches, publie `catalogRevision`, puis synchronise le catalogue du nouveau profil si aucun catalogue local n'existe, sans afficher le faux popup Xtream du quick-check. La suppression d'un profil tente aussi de supprimer la config playlist serveur seulement si elle correspond au profil supprime, afin d'eviter que le profil par defaut soit recree au prochain `device_status`.
...
- Apres splash et avant Home, un ecran `ProfilePickerScreen` affiche les profils configures sous forme d'avatars standards. OK sur un profil affiche le cercle de chargement bleu a l'emplacement de l'ancien bouton `Manage profiles`, active le profil, recharge le catalogue local si necessaire, puis ouvre Home. `Manage profiles` n'est plus affiche. `Add Profile` ouvre directement le meme popup `Ajouter un profil` que Info compte; le stylo devant le nom ouvre le meme formulaire en modification pre-remplie.
- Info compte compacte la section source: icone utilisateur bleue pour le panneau, badge d'usage dans l'en-tete Licence SmartVision, expiration Xtream dans l'en-tete Info compte, identifiants Xtream sur une ligne, boutons Xtream en icones et bascules source plus petites. La section integree `Appareil et catalogue` affiche les compteurs Live TV / Films / Series, le nom du profil catalogue actif, le Code TV et la derniere sync; elle n'affiche plus identifiant appareil, version ni portail. La progression de synchronisation est rendue dans cette section pour rester visible pendant une sync.
...
- Settings: experience video, personnalisation focus, langue, synchro, activite reseau, attribution TMDB, comptes Xtream, parental. L'ecran reutilise le header principal comme Info compte, le focus initial vise le premier item du menu, et Back depuis Settings ou Info compte retourne sur Home en gardant le focus sur l'onglet Home du header.
