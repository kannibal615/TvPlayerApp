...
- Le titre et le sous-titre Hero utilisent strictement les valeurs admin. Un slide image-only ne rend ni copie locale, ni bloc texte, ni degrade textuel.
- Le fond global conserve `ContentScale.Crop` et n'applique plus aucun voile: l'image admin est rendue dans ses couleurs originales, recadree uniquement pour remplir l'ecran. Le fond sombre natif reste le fallback en cas d'echec de chargement.
...
- Au demarrage, `AppNavigation` et le picker sont precomposes sous le splash. Le picker signale sa disponibilite seulement apres le layout d'une vraie card et une frame suivante; ses cards sont deja composees sous le splash puis apparaissent ensemble par un fondu global de `140 ms` au handoff. Le splash conserve sa duree et son statut local: l'optimisation supprime l'ecran vide apres le splash sans accelerer artificiellement celui-ci.
...
Les ecrans actifs sont routes depuis `ui/navigation/AppNavigation.kt`. Le header principal expose Home, Live TV, Movies, Series et YouTube en onglets centraux icon-first, avec pictogrammes remplis gris clair coherents (YouTube reste rouge) et cadres 2D sombres a contour fin. Chaque onglet utilise le focus TV partage, garde une hauteur visuelle de `38 dp` et adapte sa largeur a son icone plus son libelle; aucune ligne bleue ne signale la route active. Media n'est plus un onglet du header: il est accessible depuis le menu lateral Profil entre Synchronisation et Aide. Les actions globales restent dans le bloc droite, dans l'ordre Notifications, Parametres, Profil, puis la date/heure non focusable sans separateur vertical dedie. L'horloge et ses secondes sont configurables globalement dans Personnalisation et reagissent immediatement. Le bouton licence/Premium n'est plus rendu dans le header depuis le 2026-07-20; le dialogue Premium reste appele par les features verrouillees. Les ecrans de contenu affichent un QR Xtream si aucun compte n'est configure. Si un compte Xtream existe mais que la verification rapide echoue, Home reste accessible et les entrees Live TV / Movies / Series sont bloquees par popup et overlay. Pendant une verification Xtream en cours, la navigation catalogue peut rester temporairement protegee si aucun etat connecte valide n'est deja connu, mais Home et le header ne doivent afficher les warnings/overlays "Connexion indisponible" qu'apres un echec Xtream confirme.
...
- Apres le splash et avant Home, `ProfilePickerScreen` affiche tous les profils configures dans une `LazyRow` sans limite artificielle et couvre le premier rendu avant toute frame Home visible. Si Home ou le `NavHost` est encore en stabilisation, le picker reste visible au-dessus du masque opaque; ce masque ne doit jamais apparaitre seul. `Info profil`, le dialogue de sortie et l'action profil Kids reutilisent ce meme Who's Watching global, precompose au-dessus de Home. Pendant son ouverture, Home reste composee uniquement comme support cache, non interactif, sans focus, preview ou son. OK active ensuite le profil, invalide les caches Home/Xtream et met en file le travail Home si Room est vide ou l'empreinte obsolete, y compris si la carte choisie correspondait deja au profil actif. La synchronisation reseau commence apres fermeture du picker. Le reveal final attend le profil/revision cible, la synchronisation eventuellement due, les compteurs et les tendances; Continue Watching peut terminer ensuite sans bloquer. Un `HomeViewModel` unique, possede et collecte par la navigation, fournit la disponibilite. Home remplace tout `HomeUiState.profileId` obsolete par un etat cible vide/loading avant rendu. La synchronisation ne se lance que si le catalogue manque ou si 24h/48h est echu; `A chaque demarrage` n'est jamais rejoue par un retour Home.
...
- `ProfileScreen.kt` reste l orchestrateur de la route Profil. Les surfaces Licence/Premium, synchronisation, editeur de profil, panneaux playlist et utilitaires partages sont separees dans `PremiumLicenseUi.kt`, `ProfileSynchronizationUi.kt`, `PlaylistProfileEditorDialog.kt`, `PlaylistProfilePanels.kt` et `ProfileUiSupport.kt`; leurs contrats publics et parcours D-pad restent inchanges.
...
- Le hero Home ne contient plus de bouton et n'entre jamais dans le parcours D-pad. Son index est reinitialise au changement de profil/du mode Kids; image, titre et sous-titre viennent du backend sans texte local de remplacement.
...
- 2026-07-22: Splash -> Who's Watching remplace le cercle de progression par un spinner indetermine, attend une vraie card precomposee avant le handoff et revele toutes les cards ensemble sur `140 ms`. La selection conserve le centrage et l'avatar partage vers Home, mais utilise une bordure or et maintient le loader cyan `3 s` minimum avant le reveal.
...
- 2026-07-18: Who's Watching active seulement le profil et termine sa transition; aucun statut ou erreur catalogue n'est affiche sur une card profil et aucune synchronisation reseau ne part derriere le picker. Le seul loader est la couronne d'avatar de la transition selectionnee.
