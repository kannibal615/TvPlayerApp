...
- Au demarrage, le picker signale sa premiere frame a `MainActivity`; `AppNavigation` est composee sous le splash avant le handoff. La barre du splash conserve une progression locale a quatre etapes et atteint 100 % seulement quand la surface initiale est prete ou quand le timeout de securite expire.
