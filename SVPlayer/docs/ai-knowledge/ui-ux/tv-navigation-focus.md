...
- Depuis le 2026-07-05, la route `Media` utilise `MediaCatalogHeader` et un ecran 3 zones connecte au stockage local. Le focus initial va vers la bibliotheque; les zones Media demandent ensuite le focus sur le premier fichier/dossier compose. D-pad droite depuis une ligne fichier envoie le focus vers la premiere action active du panneau apercu. `Lire` est actif pour video/audio/photo et ouvre `media_player/{mediaFileId}`; dossiers, renommer, deplacer et supprimer restent focusables au D-pad.
