...
Le header principal boucle horizontalement entre son premier onglet et sa derniere action visible, dans les deux sens; en profil Kids, l'avatar est la derniere action. Depuis le 2026-07-21, ses six onglets centraux utilisent des PNG generes `ic_header_*`, tous blancs, simples et a trait homogene. Chaque onglet consomme le style partage `TvFocusStyle` via `tvFocusTarget`: le focus utilise donc le cadre, le fond et l'accent globaux, tandis que la route active conserve un fond/accent `active` distinct lorsque le focus part dans le contenu. Le label reste visible pour l'onglet actif ou focusse. Le separateur apres YouTube, le bloc droite global et tous les `FocusRequester`/callbacks D-pad restent inchanges. Home applique la meme boucle aux trois cards Categories ainsi qu'aux rangees Continue Watching et Tendances, avec scroll vers la carte opposee avant la demande de focus.
