...
- Le hero Home est strictement informatif: texte et pagination uniquement, sans CTA ni cible focusable. Un clearance non scrollable de `24 dp` le separe du header; tout retour cible vers le header remet le scroll Home en haut. Son texte vient des `SmartVisionStrings` de la langue active, meme quand l'image est distante.
...
- Home conserve un clearance non scrollable de `24 dp` entre le header partage et le hero. La relocation verticale automatique est neutralisee, les halos des cards Live TV/Series peuvent depasser horizontalement du viewport, et un gap visuel unique de `12 dp` relie chaque rangee de cards au titre de la section suivante.
