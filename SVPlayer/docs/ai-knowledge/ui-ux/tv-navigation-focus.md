...
Le header principal boucle horizontalement entre son premier onglet et sa derniere action visible, dans les deux sens; en profil Kids, l'avatar est la derniere action. Depuis le 2026-07-21, ses six onglets centraux utilisent des PNG generes `ic_header_*`, tous blancs, simples et a trait homogene. Chaque cible garde une largeur fixe `58 dp`, espacee de `12 dp`; le glyph mesure `34 dp`, sa surface focus `40 dp` et la hauteur partagee du header `52 dp` reserve la ligne basse du label. Le focus est local a l'onglet: cadre blanc arrondi, fond blanc a 9 %, scale discret `1.03` et label; aucun halo radial mobile ni trait bleu n'est rendu. Hors focus, il ne reste ni fond, cadre ni label; l'icone blanche reste a 82 %. Le separateur apres YouTube, le bloc droite global et tous les `FocusRequester`/callbacks D-pad restent inchanges. Home applique la meme boucle aux trois cards Categories ainsi qu'aux rangees Continue Watching et Tendances, avec scroll vers la carte opposee avant la demande de focus.
...
- Au premier demarrage, `AppNavigation` et Who's Watching sont precomposes derriere `StartupExperience`. Le splash attend la premiere frame du picker avant son retrait (timeout de securite `2500 ms`); le picker couvre ensuite l'ecran avant que Home soit visuellement revelee. Pendant la stabilisation `NavHost` ou une demande `openProfilePickerAfterHome`, le picker reste rendu au-dessus du masque opaque; le masque ne doit jamais etre la seule surface visible.
