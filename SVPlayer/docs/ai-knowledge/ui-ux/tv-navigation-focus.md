...
- Sans etat secondaire, Back quitte le player Live en un seul appui, que l'overlay principal soit visible ou masque. Le comportement pendant une publicite reste inchange.
...
- L'overlay Live agrandit de 40 % ses quatre icones secondaires et de 60 % Lecture/Pause, sans disque bleu plein. En mode luminosite, les cinq commandes disparaissent: seule la barre reste rendue; OK/Back la ferme puis restaure le focus Luminosite.

Le header principal boucle horizontalement entre son premier onglet et sa derniere action visible, dans les deux sens; en profil Kids, l'avatar est la derniere action. Ses cinq onglets centraux sont Home, Live TV, Movies, Series et YouTube: ils utilisent des pictogrammes remplis dans le meme gris clair que les actions globales (YouTube conserve son pictogramme rouge), un cadre 2D sombre a contour fin et un libelle toujours lisible. Leur hauteur visuelle est `38 dp`, comme Notifications et Parametres, et leur largeur enveloppe strictement icone plus libelle; aucun trait bleu de route active n'est rendu. Chaque onglet consomme le style partage `TvFocusStyle` via `tvFocusTarget`: le focus utilise donc le cadre, le fond et l'accent globaux. A droite, les controles conservent exactement leurs boutons icone-only et leur focus existants dans l'ordre Notifications, Parametres, Profil. L'acces Media quitte le header et se trouve dans le menu Profil entre Synchronisation et Aide. Le separateur apres YouTube, le bloc droite global et tous les `FocusRequester`/callbacks D-pad restent inchanges. Home applique la meme boucle aux trois cards Categories ainsi qu'aux rangees Continue Watching et Tendances, avec scroll vers la carte opposee avant la demande de focus.
...
Le bouton Home du header revient vers l'entree Home racine avec `popUpTo(Home)` et `launchSingleTop`. Le `NavHost` n'anime pas simultanement les deux ecrans complets, afin d'eviter leur double rendu sur Fire TV. Un retour chaud conserve les donnees de la paire `profileId + catalogRevision`, sans skeleton ni delai minimum; le rafraichissement explicite reste stale-while-revalidate et ne vide l'ecran qu'en cas de nouveau profil, nouvelle revision, synchronisation ou cache absent.
...
Depuis le 2026-07-22, le bloc Premium/QR sous le mini-player des ecrans Movies et Series rend le meme composable que Live TV, avec ses textes VOD: fond navy/noir radial, couronne et filets or, QR blanc borde et code TV encadre par les filets. Il reste purement informatif et non focusable.
...
- Who's Watching reutilise `startup_cinema_background` en plein ecran, comme le Splash. Haut depuis une card principale est consomme afin de conserver le focus courant; Haut depuis le crayon revient toujours a sa card. Back ouvre la confirmation de sortie avec seulement `Quitter` et `Annuler` lorsque le picker est deja affiche.
...
- Au premier demarrage, `AppNavigation` et Who's Watching sont precomposes derriere `StartupExperience`. Le splash attend qu'une vraie card soit placee puis une frame supplementaire avant son retrait (timeout de securite `2500 ms`); le picker est alors revele globalement en `140 ms`, sans decalage entre cards. Pendant la stabilisation `NavHost` ou une demande `openProfilePickerAfterHome`, le picker reste rendu au-dessus du masque opaque; le masque ne doit jamais etre la seule surface visible.
...
- La hauteur visuelle des cards Who's Watching depend du plus haut libelle mesure dans la rangee, puis est appliquee a tous les profils et aux deux actions d'ajout. Le padding vertical interne est de `20 dp`, independant du padding horizontal `14 dp`, pour laisser davantage d'air au-dessus de l'avatar et sous le nom. Le bouton crayon reste hors de la surface et son routage Haut/Bas ne change pas.
- Who's Watching boucle horizontalement entre le premier profil et `Add Profile`: Gauche depuis le premier profil scrolle vers la derniere action, Droite depuis `Add Profile` scrolle vers le premier profil puis demande le focus. Ce wrap passe par `LazyListState.scrollToItem()` avant `requestFocus()` pour eviter une cible lazy non composee. Le focus gauche/droite simple ne scale plus les cards; seul l'etat selection garde son scale anime, et les bounds ne sont mesures que pour la card selectionnee.
...
- Sur Home, D-pad bas depuis Live TV / Movies / Series cible le premier item de la prochaine ligne disponible. Entre Continue watching, Trending movies et Trending series, Haut/Bas transmet l'index source et cible la card visible dont le centre horizontal est le plus proche.
- Ce routage vertical annule le job precedent, conserve le scroll horizontal de chaque `LazyRow`, rend la cible composee si necessaire, execute un unique `ScrollState.animateScrollTo()` puis demande son `FocusRequester` stable par ID. Le reset a l'index `0` reste reserve a l'entree depuis les categories et au rafraichissement explicite Home. Le `verticalScroll` repond `0` aux demandes automatiques `BringIntoView`; ne pas reutiliser `BringIntoViewRequester` pour ces transitions.
...
- Depuis le 2026-07-17, le champ de recherche Live TV/Movies/Series intercepte `DPAD_DOWN` avant `BasicTextField`, attend que la liste filtree soit composee et restaure le dernier item pertinent encore present ou le premier resultat. Si aucun resultat n'existe, le champ conserve le focus.
...
- Le player plein ecran Live TV a un overlay dedie. D-pad Haut/Bas zappe la chaine precedente/suivante avec l'overlay principal visible ou masque; le premier `ACTION_DOWN` change la chaine et les repetitions sont consommees. Bas n'ouvre plus Parametres: seul Menu ouvre le panneau secondaire. Apres zapping, un guide gauche compact non focusable montre pendant 3 secondes jusqu'a cinq chaines dans l'ordre Room, uniquement avec logo et nom. Sans cadre global ni cadre de logo, il reste leger; la chaine courante utilise un fond discret et un repere lateral. Son etat partage entre routes evite le flash pendant un zapping repete, et chaque zap relance le delai. Les panneaux secondaires deja ouverts restent proprietaires de Haut/Bas. Gauche/droite navigue les boutons du bandeau; Back ferme d'abord les surfaces secondaires puis revient a la liste Live TV. A la sortie, la liste restaure le focus sur la derniere chaine ouverte, y compris apres zapping.
...
- Depuis le 2026-07-20, la colonne gauche de `Media` reste hierarchique uniquement pour `Media local`: parent expandable avec `All files`, `Recordings`, `Imports`, `Transfers`, puis les actions QR `Importer tel.` et `Exporter tel.` en lignes compactes. Le focus initial reste sur `Media local`; DPAD droite depuis la liste cible la preview seulement si un fichier local est selectionne.
...
- Home conserve un clearance non scrollable de `14 dp` entre le header partage et le hero, symetrique au padding haut du shell. La relocation verticale automatique est neutralisee, les halos des cards Live TV/Series peuvent depasser horizontalement du viewport, et un gap visuel unique de `12 dp` relie chaque rangee de cards au titre de la section suivante.
...
- Media selectionne explicitement le fichier local avant le transfert DPAD droite, puis rend DPAD gauche a la liste ou a la bibliotheque si la liste est vide.
- Live TV Categories: les filtres sont masques par defaut. Le premier OK sur le bouton Filtre affiche/replie la barre inline et consomme la paire Down/Up de cette activation; Bas entre dans le filtre actif et Bas rejoint ensuite la categorie selectionnee. Haut effectue le chemin inverse. `All` reste premier, la selection applique le filtre sans refermer la barre et les chips gardent leur identite stable.
...
- Le champ recherche Media reste focusable/cliquable pour ouvrir le clavier TV via OK/clic et filtre les fichiers locaux. Les lignes enfants sous `Media local` doivent rester plus basses que leur parent pour eviter la confusion visuelle. Les focus Media utilisent `LocalTvFocusStyle`.
...
- 2026-07-21: remplacement cible des six onglets du header par des vecteurs `ic_header_*`, avec modele monochrome/couleurs originales, YouTube rouge non teinte, scale `1.15`, halo radial partage mobile, barre bleue et label reserve au focus. Les routes, l'ordre, le shell `44 dp`, le bloc Notifications/Profil/Parametres et le routage D-pad restent inchanges.
...
- 2026-07-20: Media prives retire de l'app et du backend/admin; Media Center ne garde que fichiers locaux, enregistrements, imports et transferts telephone/TV.
...
- 2026-07-18: le player Live limite son overlay principal aux cinq commandes de la maquette autour de Play/Pause. Gauche/Droite parcourt Precedent, Play/Pause, Suivant, Luminosite et Sortie; depuis le 2026-07-22, seul Menu ouvre le panneau secondaire et D-pad Bas zappe. Back ferme le panneau avant de quitter, et les touches media rewind/fast-forward conservent le seek quand le flux le permet.
