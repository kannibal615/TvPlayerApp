...
Le header principal boucle horizontalement entre son premier onglet et sa derniere action visible, dans les deux sens; en profil Kids, l'avatar est la derniere action. Depuis le 2026-07-21, ses six onglets centraux utilisent les `VectorDrawable` 24 x 24 `ic_header_*`: Home, Live TV, Movies, Series et Media sont monochromes, tandis que YouTube conserve son rectangle rouge et son triangle blanc avec `Color.Unspecified`. Chaque cible garde une empreinte fixe `58 x 44 dp`, espacee de `12 dp`; seule la couche dessinee passe a `1.15x` via `graphicsLayer`. Le focus affiche simultanement un halo radial bleu partage qui glisse entre les bounds mesures, une barre `28 x 3 dp`, l'icone a pleine opacite et le label anime. Hors focus, aucun fond, cadre, halo, barre ou label ne reste visible, y compris pour la route active; les icones monochromes sont a 80 % et YouTube reste rouge. Le separateur apres YouTube, le bloc droite global et tous les `FocusRequester`/callbacks D-pad restent inchanges. Home applique la meme boucle aux trois cards Categories ainsi qu'aux rangees Continue Watching et Tendances, avec scroll vers la carte opposee avant la demande de focus.
